...
function createResoListingsProvider(config = {}) {
  ...
  return {
    ...
    async fetchListings({ top = 50, skip = 0, filter = "StandardStatus eq 'Active'" } = {}) {
      if (!ready) throw userError("PROVIDER_MODE_DISABLED");
      const data = await odataGet(`/Property?$filter=${encodeURIComponent(filter)}&$top=${top}&$skip=${skip}`);
